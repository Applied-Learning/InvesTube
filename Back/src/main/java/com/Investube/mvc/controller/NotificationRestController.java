...
public class NotificationRestController {
    ...
    @GetMapping("/settings")
    public ResponseEntity<Map<String, Boolean>> getSettings(@RequestHeader(HttpHeaders.AUTHORIZATION) String authorizationHeader) {
        String token = authorizationHeader.replace("Bearer ", "");
        Integer userId = jwtUtil.getUserIdByToken(token);
        if (userId == null) return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        Map<String, Boolean> settings = notificationService.getSettingsForUser(userId);
    ...
    public ResponseEntity<Void> updateSettings(
        ...
        Integer userId = jwtUtil.getUserIdByToken(token);
        if (userId == null) return new ResponseEntity<>(HttpStatus.UNAUTHORIZED);
        notificationService.saveSettingsForUser(userId, settings);
        return new ResponseEntity<>(HttpStatus.OK);
    }
}
